...
// A tool is "out" when someone has been named on the latest transfer.
const isOut = (t) => !!t.issued_to;
const isOverdue = (t) => !!t.issued_to && !!t.due_date && t.due_date < todayISO();

function daysLate(t) {
  if (!isOverdue(t)) return 0;
  return Math.round((new Date(todayISO()) - new Date(t.due_date)) / 86400000);
}
...
// Sending onward requires holding it.
const canSendTool = (t) => iHoldTool(t);

// Returning goes back to the tool's own home, and only the holder can do it.
const canReturnTool = (t) => iHoldTool(t) && t.current_workshop_id !== t.home_workshop_id;
...
function boot() {
  ...
  if (stamp) stamp.textContent = `Build 17 · ${CLEAN_URL.replace('https://', '')}`;
...
function filteredTools() {
  return workshopScopedTools().filter(t => {
    ...
    if (c === 'overdue' && !isOverdue(t)) return false;
    if (c === 'repair' && (!t.condition || t.condition === 'working')) return false;
    if (state.toolSearch) {
...
function renderDashboard() {
  ...
  const visibleWorkshops = state.profile.role === 'manager'
    ? state.workshops.filter(w =>
        w.id === state.profile.home_workshop_id ||
        state.tools.some(t => t.current_workshop_id === w.id))
    : state.workshops;
  ...
  const overdue = tools.filter(isOverdue).sort((a, b) => a.due_date.localeCompare(b.due_date));
  const faulty = tools.filter(t => t.condition && t.condition !== 'working');
  const incoming = tools.filter(canReceiveTool);
  const inTransit = tools.filter(isInTransit);
  ...
  el.innerHTML = `
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card"><div class="kpi-num">${tools.length - out.length}</div><div class="kpi-label">At home</div></div>
      <div class="kpi-card"><div class="kpi-num">${out.length}</div><div class="kpi-label">Away</div></div>
      <div class="kpi-card ${incoming.length ? 'kpi-info' : ''}"><div class="kpi-num">${incoming.length}</div><div class="kpi-label">To confirm</div></div>
      <div class="kpi-card ${overdue.length ? 'kpi-alert' : ''}"><div class="kpi-num">${overdue.length}</div><div class="kpi-label">Overdue</div></div>
      <div class="kpi-card ${faulty.length ? 'kpi-warn' : ''}"><div class="kpi-num">${faulty.length}</div><div class="kpi-label">Needs repair</div></div>
      ${state.profile.role === 'manager'
        ? `<div class="kpi-card"><div class="kpi-num">${tools.filter(isAwayFromMySite).length}</div><div class="kpi-label">Sent out from here</div></div>`
        : ''}
    </div>

    ${incoming.length ? `
      <div class="section-title section-info">🚚 Waiting for you to confirm — ${incoming.length}</div>
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>Tool</th><th>From</th><th>Sent by</th><th></th></tr></thead>
          <tbody>
            ${incoming.map(t => `<tr>
              <td>${escapeHtml(t.name)} <span class="mono">${escapeHtml(t.serial_number || '')}</span></td>
              <td>${escapeHtml(t.home?.name || '—')}</td>
              <td>${escapeHtml(t.issued_to || '—')}</td>
              <td><button class="btn btn-ok btn-sm" data-receive="${t.id}">Mark received</button></td>
            </tr>`).join('')}
          </tbody>
        </table>
      </div>` : ''}

    ${inTransit.length && !incoming.length ? `
      <div class="section-title">In transit — ${inTransit.length}</div>
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>Tool</th><th>Heading to</th><th>Status</th></tr></thead>
          <tbody>
            ${inTransit.slice(0, 15).map(t => `<tr>
              <td>${escapeHtml(t.name)} <span class="mono">${escapeHtml(t.serial_number || '')}</span></td>
              <td>${escapeHtml(t.workshop?.name || '—')}</td>
              <td class="mono">Awaiting confirmation</td>
            </tr>`).join('')}
          </tbody>
        </table>
      </div>` : ''}
    ...
    ${overdue.length ? `
      <div class="section-title section-alert">⚠ Overdue — ${overdue.length} tool${overdue.length > 1 ? 's' : ''}</div>
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>Tool</th><th>With</th><th>Where</th><th>Was due</th><th>Late by</th></tr></thead>
          <tbody>
            ${overdue.map(t => `<tr class="row-alert">
              <td>${escapeHtml(t.name)} <span class="mono">${escapeHtml(t.serial_number || '')}</span></td>
              <td>${escapeHtml(t.issued_to)}</td>
              <td>${escapeHtml(t.workshop?.name || '—')}</td>
              <td class="mono">${formatDate(t.due_date)}</td>
              <td><strong>${daysLate(t)} day${daysLate(t) === 1 ? '' : 's'}</strong></td>
            </tr>`).join('')}
          </tbody>
        </table>
      </div>` : ''}
    ...
    ${out.length ? `
      <div class="section-title">Away from home</div>
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>Tool</th><th>With</th><th>Where</th><th>Due back</th></tr></thead>
          <tbody>
            ${out.slice(0, 25).map(t => `<tr>
              <td>${escapeHtml(t.name)} <span class="mono">${escapeHtml(t.serial_number || '')}</span></td>
              <td>${escapeHtml(t.issued_to || '—')}</td>
              <td>${escapeHtml(t.workshop?.name || '—')}</td>
              <td class="mono ${isOverdue(t) ? 'text-alert' : ''}">${formatDate(t.due_date)}</td>
            </tr>`).join('')}
          </tbody>
        </table>
      </div>` : `<div class="section-title">Away from home</div><p style="color:var(--ink-faint);font-size:14px;">Everything is at its home workshop.</p>`}
  ...
  el.innerHTML = `
    ...
    <div class="toolbar">
      ...
      <select id="toolCustodySelect">
        ...
        <option value="overdue" ${state.toolCustodyFilter === 'overdue' ? 'selected' : ''}>Overdue</option>
        <option value="repair" ${state.toolCustodyFilter === 'repair' ? 'selected' : ''}>Needs repair</option>
      </select>
    </div>
...
function renderToolResults() {
  ...
  const isFiltered = state.toolSearch || state.toolStatusFilter || state.toolCustodyFilter;
  ...
  return `
    <div class="tool-tag ${state.selectMode ? 'selectable' : ''} ${selected ? 'selected' : ''}" data-id="${t.id}">
      ...
      <div class="tool-tag-body">
        ...
        ${locationLine(t)}
        ${custodyLine(t)}
        ${t.condition && t.condition !== 'working'
          ? `<div class="cond-flag cond-${t.condition}">${CONDITION_LABEL[t.condition]}</div>` : ''}
      </div>
    </div>`;
}
...
function custodyLine(t) {
  ...
  return `<div class="custody ${late ? 'custody-late' : ''}">
    👤 ${escapeHtml(t.issued_to)} · ${late
      ? `<strong>${daysLate(t)} day${daysLate(t) === 1 ? '' : 's'} overdue</strong>`
    ...
    <form id="toolForm">
      ...
      <label class="field"><span>Condition</span>
        <select name="condition">
          ${Object.entries(CONDITION_LABEL).map(([k, v]) => `<option value="${k}">${v}</option>`).join('')}
        </select>
      </label>
...
async function openToolDetail(id) {
  ...
  openModal(`
    ...
    ${isOut(t) ? `<div class="custody-box ${isOverdue(t) ? 'custody-box-late' : ''}">
        <div><span class="cred-label">Issued to</span><span class="cred-value">${escapeHtml(t.issued_to)}</span></div>
        <div><span class="cred-label">Due back</span><span class="cred-value">${formatDate(t.due_date)}${isOverdue(t) ? ` — ${daysLate(t)} day${daysLate(t) === 1 ? '' : 's'} overdue` : ''}</span></div>
      </div>` : ''}
...
function openBulkTransferModal() {
  ...
  openModal(`
    ...
    <form id="bulkTransferForm" style="margin-top:18px;">
      ...
      <label class="field"><span>Issued to (person taking them)</span>
        <input name="issued_to" type="text" placeholder="Name of the person" required>
      </label>
      <label class="field"><span>Due back</span>
        <input name="due_date" type="date" value="${defaultDue}" min="${todayISO()}" required>
      </label>
  ...
  document.getElementById('bulkTransferForm').addEventListener('submit', async (e) => {
    ...
    const issuedTo = (fd.get('issued_to') || '').trim() || null;
...
function openTransferModal(t) {
  ...
  openModal(`
    ...
    <form id="transferForm">
      ...
      <label class="field"><span>Issued to (person taking it)</span>
        <input name="issued_to" type="text" placeholder="Name of the person" required>
      </label>
      <label class="field"><span>Due back</span>
        <input name="due_date" type="date" value="${defaultDue}" min="${todayISO()}" required>
  ...
  document.getElementById('transferForm').addEventListener('submit', async (e) => {
    ...
    const payload = {
      ...
      issued_to: (fd.get('issued_to') || '').trim() || null,
...
function renderLog() {
  ...
  el.innerHTML = `
    ...
    <div class="table-wrap">
      <table class="data-table">
        <thead><tr><th>Tool</th><th>From</th><th>To</th><th>Issued to</th><th>Due</th><th>By</th><th>When</th><th>Note</th></tr></thead>
        <tbody>
          ${rows.length ? rows.map(t => `
            <tr>
              ...
              <td>${t.issued_to ? escapeHtml(t.issued_to) : '<span class="mono">returned</span>'}</td>
              <td class="mono">${t.due_date ? formatDate(t.due_date) : '—'}</td>
              <td>${escapeHtml(t.by?.full_name || '—')}</td>
              <td class="mono">${formatDateTime(t.transferred_at)}</td>
              <td>${escapeHtml(t.note || '')}</td>
            </tr>`).join('') : `<tr><td colspan="8" style="color:var(--ink-faint)">No transfers recorded yet.</td></tr>`}
        </tbody>
      </table>
    </div>
  `;
...
function renderWorkshops() {
  ...
  el.innerHTML = `
    ...
    <div class="table-wrap">
      <table class="data-table">
        <thead><tr><th>Name</th><th>Address</th><th>Tools homed here</th><th>Physically here</th></tr></thead>
        <tbody>
          ${state.workshops.map(w => `
            <tr>
              <td>${escapeHtml(w.name)}</td>
              <td>${escapeHtml(w.address || '—')}</td>
              <td>${state.tools.filter(t => t.home_workshop_id === w.id).length}</td>
              <td>${state.tools.filter(t => t.current_workshop_id === w.id).length}</td>
            </tr>`).join('') || `<tr><td colspan="4" style="color:var(--ink-faint)">No workshops yet.</td></tr>`}
        </tbody>
      </table>
    </div>
  `;
