...
// ---------------------------------------------------------------------------
function getConfig() {
  const url = localStorage.getItem('toolroom_url');
  const key = localStorage.getItem('toolroom_key');
...
document.getElementById('cfgSave').addEventListener('click', () => {
  ...
  if (!url || !key) return;
  localStorage.setItem('toolroom_url', url);
  localStorage.setItem('toolroom_key', key);
  document.getElementById('configScreen').classList.add('hidden');
  startSupabase(url, key);
});
...
function filteredTools() {
  return state.tools.filter(t => {
    if (state.workshopFilter && t.current_workshop_id !== state.workshopFilter) return false;
...
function renderTools() {
  ...
  el.innerHTML = `
    ...
    ${tools.length ? `<div class="tool-grid">${tools.map(toolCard).join('')}</div>` :
      `<div class="empty-state"><h3>No tools match</h3><p>Try clearing filters${canWrite ? ', or add the first tool.' : '.'}</p></div>`}
  `;
  ...
  document.getElementById('toolSearchInput').addEventListener('input', (e) => {
    ...
    const v = document.getElementById('toolSearchInput');
