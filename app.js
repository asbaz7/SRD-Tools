...
function boot() {
  ...
  if (stamp) stamp.textContent = `Build 10 · ${CLEAN_URL.replace('https://', '')}`;
...
function startSupabase(url, key) {
  ...
  // A password-reset email sends the user back here with a recovery token in
  // the URL. Detect that first, so we show the "set a new password" screen
  // instead of briefly flashing the signed-in app.
...
document.getElementById('forgotBtn').addEventListener('click', async () => {
  ...
  const errEl = document.getElementById('loginError');
  if (!email) {
    errEl.textContent = 'Type your email address above first, then click this again.';
    errEl.classList.remove('hidden');
  ...
  errEl.classList.add('hidden');
  showToast('If that email has an account, a reset link is on its way. Check spam too.');
});
...
document.getElementById('resetForm').addEventListener('submit', async (e) => {
  ...
  // Clear the recovery token out of the address bar, then send them to sign in.
...
function openAccountModal() {
  ...
  openModal(`
    ...
    <label class="field"><span>New password</span>
      <input id="acctPw1" type="password" autocomplete="new-password" minlength="8">
    </label>
    <label class="field"><span>Confirm new password</span>
      <input id="acctPw2" type="password" autocomplete="new-password" minlength="8">
    </label>
    <button type="button" class="btn btn-primary btn-block" id="savePwBtn">Update password</button>
    <p id="acctError" class="form-error hidden"></p>

    <div class="modal-actions" style="margin-top:26px;justify-content:space-between;">
...
async function afterLogin() {
  ...
  // Managers only ever see their own workshop, so the selector would be
  // misleading. Replace it with a fixed label of where they are.
  const sel = document.getElementById('workshopFilter');
  if (profile.role === 'manager') {
    const home = state.workshops.find(w => w.id === profile.home_workshop_id);
    sel.innerHTML = `<option value="">${escapeHtml(home ? home.name : 'No workshop assigned')}</option>`;
    sel.disabled = true;
    if (!profile.home_workshop_id) {
...
function openAddToolModal() {
  // A manager can only add tools at their own site, so don't offer choices
  ...
  openModal(`
    ...
    <form id="toolForm">
      ...
      <label class="field"><span>Workshop</span>
        <select name="current_workshop_id" required ${isManager ? 'disabled' : ''}>
  ...
  document.getElementById('toolForm').addEventListener('submit', async (e) => {
    ...
    // A disabled select isn't included in form data, so fill it in.
    if (isManager) payload.current_workshop_id = state.profile.home_workshop_id;
    if (!payload.current_workshop_id) return showToast('No workshop assigned — ask an admin.');
