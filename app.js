...
function boot() {
  ...
  if (stamp) stamp.textContent = `Build 18 · ${CLEAN_URL.replace('https://', '')}`;
...
function renderDashboard() {
  ...
  el.innerHTML = `
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card ${longOut.length ? 'kpi-alert' : ''}"><div class="kpi-num">${longOut.length}</div><div class="kpi-label">Out too long</div></div>
    ...
    ${incoming.length ? `
      ...
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>Tool</th><th>Home workshop</th><th></th></tr></thead>
          <tbody>
            ${incoming.map(t => `<tr>
    ...
    ${longOut.length ? `
      <div class="reminder-banner">
        <strong>Reminder</strong> — ${longOut.length} tool${longOut.length > 1 ? 's have' : ' has'} been away
        longer than ${LOAN_ALERT_DAYS} days. Worth checking whether ${longOut.length > 1 ? "they're" : "it's"} still needed.
      </div>
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>Tool</th><th>Where it is</th><th>Left home</th><th>Time out</th></tr></thead>
          <tbody>
            ${longOut.map(t => `<tr class="row-alert">
              <td>${escapeHtml(t.name)} <span class="mono">${escapeHtml(t.serial_number || '')}</span></td>
    ...
    ${out.length ? `
      ...
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>Tool</th><th>Home</th><th>Where it is</th><th>Time out</th></tr></thead>
          <tbody>
            ${out.slice(0, 30).map(t => `<tr>
              <td>${escapeHtml(t.name)} <span class="mono">${escapeHtml(t.serial_number || '')}</span></td>
              <td>${escapeHtml(t.home?.name || '—')}</td>
              <td>${escapeHtml(t.workshop?.name || '—')}${t.in_transit ? ' <span class="mono">(in transit)</span>' : ''}</td>
